...
#}#!/bin/sh
# Base16 tokyo night night - Shell color setup script
# enika
...
color08="32/34/4A" # Base 03 - Bright Black
...
color18="23/24/33" # Base 01
color19="2A/2B/3D" # Base 02
color20="3B/3D/57" # Base 04
...
if [ -n "$ITERM_SESSION_ID" ]; then
  ...
  printf $printf_template_custom Ph 1A1B26 # background
  printf $printf_template_custom Pi A9B1D6 # bold color
  printf $printf_template_custom Pj 2A2B3D # selection color
  printf $printf_template_custom Pk A9B1D6 # selected text color
  printf $printf_template_custom Pl A9B1D6 # cursor
  printf $printf_template_custom Pm 1A1B26 # cursor text
else
