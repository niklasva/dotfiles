...
** shr
#+begin_src emacs-lisp
  (defun niva/insert-image (image &optional alt _area _slice)
    (let ((height (cdr (image-size image t))))
      (insert-sliced-image image alt nil (max 1 (/ height 20.0)) 1)))

  (defun niva/put-sliced-image (spec alt &optional flags)
    (niva/insert-image spec alt)
    (shr-put-image spec alt flags))

  (defun niva/render-image-tag-without-underline (dom &optional url)
    (let ((start (point)))
      (shr-tag-img dom url)
      (put-text-property start (point) 'face '(:underline nil))))

  (defun niva/setup-image-rendering ()
    (interactive)
    (setf shr-put-image-function #'niva/+rss-put-sliced-image-fn
          shr-external-rendering-functions '((img . niva/+rss-render-image-tag-without-underline-fn))))

  (add-hook 'elfeed-show-mode-hook 'niva/setup-image-rendering)
#+end_src
** eww
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package circe
    ...
    (defun niva/privmsg (nick userhost _command target text)
      (message (format " %s@%s" nick target))
