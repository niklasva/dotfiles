...
#+begin_src emacs-lisp
  (use-package org
    :straight t
    :config
    (defun niva/disable-org-font-height ()
      (interactive)
...
#+begin_src emacs-lisp
  (with-eval-after-load 'circe
    ...
    (defvar niva--irc-busynil)
    (defun niva/privmsg (nick userhost _command target text)
      (niva/log-to-buffer " irc://history" (format "%s %s\n%s %s\n"
                                                   (propertize (format-time-string "[%H:%M:%S]") 'face 'font-lock-comment-face)
                                                   (propertize target 'face 'lui-irc-colors-fg-10-face)
                                                   (propertize (format "%s:" nick) 'face 'font-lock-string-face)
                                                   text))
    ...
    (advice-add 'switch-to-buffer :around #'niva/remove-irc-notification-if-read))
#+end_src

#+begin_src emacs-lisp
  (defun niva/log-to-buffer (buffer text)
