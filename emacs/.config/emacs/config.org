      ...
      (lambda (src-block)
        (when (string= "emacs-lisp" (org-element-property :language src-block))
          (let* ((begin (org-element-property :begin src-block))
            ...
            (message "Formatted code block from %d to %d" begin end)))))
    (message "Formatted all Emacs Lisp code blocks in the buffer."))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun elfeed-search-format-date (date)
    ...
    (setq-local date-yesterday (concat (format-time-string "%Y%m") dom-yesterday))
  ...
  (defun niva/elfeed-search-print-entry (entry)
    ...
    (setq-local tags-str (format (concat "%" (format "%ss" (- 40 feed-ln))) (mapconcat 'identity tags " ")))
