...
(use-package git-gutter
  ...
  (setq-default git-gutter:update-interval 1))
...
(use-package git-gutter-fringe
  ...
  (define-fringe-bitmap 'git-gutter-fr:deleted [192] nil nil '(center repeated)))
#+end_src
...
(defun niva/elfeed-search-print-entry (entry)
  (let* ((feed (elfeed-entry-feed entry))
         ...
         ;; (unread (if (member "unread" (mapcar #'symbol-name (elfeed-entry-tags entry))) "* " ""))
         (tags (delete "unread" (delete "star" (mapcar #'symbol-name (elfeed-entry-tags entry)))))
         (tags-str "%-12s ")
         (date (format "%-12s " (relative-date  (elfeed-entry-date entry))))
         (title (or (elfeed-meta entry :title) (elfeed-entry-title entry) ""))

         (title-faces      (elfeed-search--faces-2 (elfeed-entry-tags entry)))
         ...
         (formatted-date (propertize (format "%s" date) 'face 'elfeed-search-title-face))
         (formatted-tags (propertize (format tags-str (mapconcat 'identity tags " ")) 'face 'elfeed-search-title-face))



         (formatted-feed-title (propertize feed-title                  'face feed-title-faces))

         (title-width-wide (- (window-width) (string-width formatted-date) (string-width formatted-tags) widest-feed-title 5))
         (title-width (- (window-width) (string-width formatted-date)))
         ;;(formatted-title      (propertize (format (format "%%%ds" title-width-lg) title) 'face title-faces 'kbd-help title)))
         ...
         ;; (title-column-lg (elfeed-format-column title title-width-lg :left))
         ;;(formatted-title      (propertize (format "%s" title-column-lg) 'face title-faces 'kbd-help title)))
)

    (setq pre-tag-padding (format (format "%%%ds" (- widest-feed-title (string-width formatted-feed-title))) "" ))
    (setq end-of-window (format (format "%%%ds"
                                        (- (window-width)
                                           ;;(string-width formatted-date)
    ...
    (setq pre-title-padding (format (format "%%%ds" (string-width formatted-date)) "" ))
