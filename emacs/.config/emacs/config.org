...
(defun niva/gptel-auto-save ()
  ...
  (when (bound-and-true-p gptel-mode)
    (let* ((dir "~/.gptel-chats/")
           ...
           (filename
            (if (string-match-p "^\\*gptel" buf-name)
                (let ((new-name (format "chat-%s" (format-time-string "%Y-%m-%d_%H-%M-%S"))))
                  (rename-buffer new-name)
                  (expand-file-name (concat new-name ".org") dir))
              (expand-file-name (concat buf-name ".org") dir))))
