...
#+begin_src emacs-lisp
  ...
  (setq-default mode-line-format
                '((:eval (if (and buffer-file-name (buffer-modified-p)) "*%b" " %b"))
                  ...
                  " | %l:%c"
                  (:eval (propertize " " 'display (list 'space :align-to (- (window-total-width) 15))))
                  (:eval (if (niva/bottom-right-window-p)
                             (format "%3d   %s" niva-elfeed-unread-count (format-time-string "%R"))))))
#+end_src
