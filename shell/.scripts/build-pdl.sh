#!/bin/sh
CWD=$PWD
PROJ_WD="/Users/niklas/dev/git/nrf52-application"
BUILD_WD="BUILD/PDL_DEBUG/GCC_ARM-MBED_PROFILE_DEBUG"

cd $PROJ_WD

sed -i '' "s/#define\ BOARD_TYPE.*/#define\ BOARD_TYPE\ \(SWE_SK_1_01\)/g" $PROJ_WD/src/board/boardType.h 

if mbed compile --profile=mbed_profile_debug.json --toolchain GCC_ARM --target PDL_DEBUG --color -l NRF52840.ld ; then
    echo $'OK\n'
else
